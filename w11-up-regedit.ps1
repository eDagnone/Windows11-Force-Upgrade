...
if ((Test-NetConnection 8.8.8.8 -InformationLevel "Quiet" -WarningAction SilentlyContinue) -eq $true) {
	Write-host "Please disconnect from the internet. Unplug your ethernet cable, and/or disconnect from wifi.`n`nIf you wish to ignore this advice, paste the following command into windows search:`n$PSCommandPath\setup.exe`nand close this script."
	while ((Test-NetConnection 8.8.8.8 -InformationLevel "Quiet" -WarningAction SilentlyContinue) -eq $true) {
		start-Sleep -Seconds 0.5
	}
}

write-host "Running $(Split-Path -Parent $PSCommandPath)setup.exe."

start-process "$(Split-Path -Parent $PSCommandPath)setup.exe"